...
每个模型约 51 GiB。本方案固定使用 8 张卡进行张量并行，但 4 个模型不同时加载：先用全部 8 张卡完成 E1，再释放资源并依次运行 E2、E5-780 和 E5-975。每张图片因此经过 4 次完整模型推理。
...
`--devices` 必须提供 8 个互不重复的设备编号。每个模型均看到这 8 张卡，并以 `tensor_parallel_size=8` 运行；当前模型进程完全退出后，脚本才启动下一个模型。输出文件必须尚未存在，避免误覆盖已有结果。
