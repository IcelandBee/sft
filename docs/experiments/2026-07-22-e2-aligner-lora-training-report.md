...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Base Qwen3.5-27B（严格 JSON） | 18 | 40 | 87 | 55 | 31.03% | 61.27% | 36.50% | 22.09% |
| E1 checkpoint-1248 | 39 | 19 | 9 | 133 | 67.24% | 6.34% | 86.00% | 73.58% |
| **E2 checkpoint-1248** | **43** | **15** | **5** | **137** | **74.14%** | **3.52%** | **90.00%** | **81.13%** |

Base 原始推理结果在修正版 Dev 上重新计分后，严格输出协议的 JSON envelope、payload JSON、完整 schema 合法率分别为 100%、82.50%、62.50%；不要求完整 schema 的 decision-only 辅助口径为 TP 18、FN 40、FP 59、TN 83，Recall 31.03%、FPR 41.55%、Accuracy 50.50%、F1 26.67%，其中 35 条输出不可用。该辅助口径只用于区分格式失败与分类失败，不与 LoRA 的严格协议结果直接混用。

为排除 Base 输出格式失败的影响，另以 Transformers token-trie 将输出限制为固定 GOOD/BAD JSON 二选一。该格式受控 Base 在同一修正版 Dev 上为 TP 19、FN 39、FP 29、TN 113，Recall 32.76%、FPR 20.42%、Accuracy 66.00%、F1 35.85%，payload JSON 与完整 schema 合法率均为 100%。合法率由解码约束保证，不代表 Base 自身的格式遵循能力。

E2 相比自由生成 Base 严格口径多识别 25 张 BAD、少误报 82 张 GOOD，Accuracy 提升 53.50 个百分点；相比格式受控 Base，E2 的 Recall 高 41.38 个百分点、FPR 低 16.90 个百分点、Accuracy 高 24.00 个百分点、F1 高 45.28 个百分点。后者因解码协议不同，只作为排除格式问题后的补充参考，不作为严格的 LoRA 因果增益。相比 E1，E2 多识别 4 张 BAD，同时少误报 4 张 GOOD，共减少 8 个错误。E2 达到 Recall≥72%、FPR≤25%、Accuracy≥72% 的 Go 标准，但 Recall 尚未达到 78% 挑战目标。
...
3. **E2 的提升不只是输出格式收益。** 自由生成 Base 的完整 schema 合法率仅 62.50%；即使通过 token-trie 将 Base 的 schema 合法率强制提升到 100%，其 Accuracy 也只有 66.00%，仍明显低于 E2 的 90.00%。由于两者解码协议不同，该结果用于证明格式失败不能解释全部差距，而不用于精确估计 LoRA 增益。
4. **剩余问题集中在细粒度视觉边界。** E2 仍有 15 个 FN，其中手部异常 8 个；5 个 FP 全部被预测为手部异常。
5. **不是简单的类别数量不足。** Train 已包含 965 张手部 BAD，但仍同时出现手部漏检和误报，更像是手指、小结构及遮挡场景的视觉区分不足。
6. **修正指标仍是开发口径。** 本次只定向复核了 E1/E2 的 57 张重点样本，其余 143 张未逐张重标；最终泛化能力仍需由修正后的隔离 Test 验证。
