...
TRAIN=/home/data/h30082292/data/pose/artifact_detection_training/ms_swift/e1_broad_clean_json_v1/train.jsonl
DEV=/home/data/h30082292/data/pose/artifact_detection_training/ms_swift/dev_adjudicated_v1/dev.jsonl
DEV_MANIFEST=/home/data/h30082292/data/pose/artifact_detection_training/ms_swift/dev_adjudicated_v1/manifest.json
OUT=/home/data/h30082292/data/pose/artifact_detection_training/runs/e3_broad_clean_vit_aligner_r16_e2_v1
MODE="${1:-run}"
...
python - "$TRAIN" "$DEV" "$DEV_MANIFEST" <<'PY'
import hashlib
import json
import sys
...
train_path, dev_path, manifest_path = map(Path, sys.argv[1:])
...
if Counter(decision(row) for row in train) != {"GOOD": 6074, "BAD": 3904}:
    raise ValueError("unexpected Train decision counts")
if Counter(decision(row) for row in dev) != {"GOOD": 142, "BAD": 58}:
    raise ValueError("unexpected adjudicated Dev decision counts")
if dev_sha256 != expected_dev_sha256:
    raise ValueError(f"unexpected Dev sha256: {dev_sha256}")
if manifest.get("dev_sha256") != expected_dev_sha256:
    raise ValueError("Dev manifest hash mismatch")
if manifest.get("training_forbidden") is not True:
    raise ValueError("Dev manifest must forbid training use")
...
echo "E3_PROTOCOL: broad-clean BADx2, LLM+aligner+ViT all-linear LoRA r16, 2 epochs"

if [[ "$MODE" == "--preflight-only" ]]; then
    echo "E3_PREFLIGHT_CHECK: PASS"
    exit 0
fi
